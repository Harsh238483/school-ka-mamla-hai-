...
const TeacherManager = () => {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      setDepartments(JSON.parse(savedData));
    } else {
  ...
  const saveToStorage = (data: Department[]) => {
    localStorage.setItem('royal-academy-teachers', JSON.stringify(data));
    setDepartments(data);
  };
  ...
  const updateDepartment = (oldName: string, newName: string, newColor: string) => {
    const updatedDepartments = departments.map(dept => {
    ...
    localStorage.setItem('royal-academy-teachers', JSON.stringify(updatedDepartments));
  ...
  const moveTeacherToDepartment = (teacherId: string, fromDepartment: string, toDepartment: string) => {
    const updatedDepartments = departments.map(dept => {
